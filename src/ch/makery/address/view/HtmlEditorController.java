...
import ch.makery.address.util.LZString;
...
public class HtmlEditorController {
	...
	/**
	 * Encode sent email data
	 * @param machineId is the machine's Mac Address
	 * @param userId is the current logged in user's ID
	 */
	public String encodeEmailData(String userId, String machineId, String emailId){
		//TODO
		String toBeEncoded = userId + ";" + machineId +";"+emailId;
		...
		//<userId>;<machineId>;<sentEmailId>
		return correctEncodedValue;
	}
	
	/**
	 * Decode encoded sent email data
	 * @param encodedValue is the encoded email data
	 */
	public String decodeEmailData(String encodedValue){
		LZString lzString = new LZString();
		String decodedValue = lzString.decompress(encodedValue);
		return decodedValue;
	}
	
	public String addEncodedEmailData(String oldEmailTemplate, String encodedValue, String meteorServerUrl){
		String meteorUrl = "http://"+meteorServerUrl+"/"+encodedValue+"/1.png";
		String newEmailTemplate = "";
		return newEmailTemplate;
	}

	@FXML
	private void addEmailDataString() {
		try {
			String encodedValue = encodeEmailData("userId","myMachindId","testEmailId");
			String imageSource = "<img src=\""
					+ "http://127.0.0.1:3000/"+encodedValue+"/1.png" + "\" width=\"1\" height=\"1\" />" + "</p>";
			WebView webView = (WebView) htmlEditor.lookup("WebView");
			WebPage webPage = Accessor.getPageFor(webView.getEngine());
			String fullHtml = htmlEditor.getHtmlText();
			int index = fullHtml.indexOf("<head>")+"<head>".length();
			System.out.println("index: "+index);
			StringBuilder builder = new StringBuilder(fullHtml);
			builder.insert(index, imageSource);
